...
```{r}
life_history %>%
  count(order,sort=T) %>% #functions as arrange
...
life_history %>%
  ...
  ggplot(aes(order,mean_mass))+geom_col()+coord_flip()
```
...
life_history %>%
  group_by(order)%>%
  summarize(mean_mass=mean(mass,na.rm=T))%>%
  ggplot(aes(order,mean_mass))+geom_col()+coord_flip()+scale_y_log10()
```
...
life_history %>%
  group_by(order)%>%
  summarize(mean_mass=mean(mass,na.rm=T))%>%
  ggplot(aes(x=reorder(order,mean_mass),y=mean_mass))+geom_col()+coord_flip()+scale_y_log10() #reoder allows us to reorder x axis bym ean mass
```
...
```{r}
life_history %>%
  ggplot(aes(gestation,wean_mass))+geom_point(na.rm=T)+scale_y_log10()
```
...
```{r}
life_history %>%
  ggplot(aes(mass,order))+
  geom_boxplot(na.rm=T)+ scale_x_log10()
 #coord_flip()
```
