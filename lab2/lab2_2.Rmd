...
```
```{r}
my_vector_sequence[my_vector_sequence<=10] # will look inside myvectorsequence and only retur those less than equal to 10 
```
...
```{r}
Earnings<-colSums(harry_potter_matrix)
Earnings
```
...
```{r}
Final_matrix<-rbind(harry_potter_matrix,Earnings)
Final_matrix
```
