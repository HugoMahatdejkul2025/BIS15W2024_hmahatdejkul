...
```{r}
k<-venom_GT+chiron+veyron
meanofspeed<-k/3
```

3. Use the function `mean()` to calculate the mean top speed for the cars.
```{r}
L<-c(venom_GT,chiron,veyron) #c will concactanate 
mean(L)
...
The value is not calculatable because one of our data points is missing.
This means NA takes up a data space but is just empty, or missing. it has no value
...
```
we removed the na. this s useful because it allows us to account for †he data that we do have
