...
```r
glimpse(amniota) # NA are -999,-999.0, -999.00...
```
...
```r
#amniota has 0 NA which doesn't make any sense. This is because it is all in the form of -999
amphibio %>%
  map_df(~ sum(is.na(.)))
```
...
```r
#amphibio has lots of NAs
```

**5. Make any necessary replacements in the data such that all NA's appear as "NA".**   

```r
amniota<-read.csv("data/amniota.csv",na=c("-999"," "))
```
...
```r
#the mammals would make sense, as they don't lay eggs. However, the reptilia and Aves might be problematic, although it is possible that we cannot obtain specific eggs to get the mass, but overall there is a lot of issues with us not having the data. This could mean that the data is incomplete and the NA's could mean that there is missing data rather than data not existing because it's impossible to collect.
```
...
```r
  #I think that the NAs represent the organism not being apart of that group. For example, something like an anura is a frog like organism, so it wouldn't acctually burrow, but it is terrestrial, aquatic and arboreal. SO i think that the Nas represent "no" or 0
```

**10. Now that we know how NA's are represented in the `amniota` data, how would you load the data such that the values which represent NA's are automatically converted?**

```r
amniota2<-read_csv("data/amniota.csv",na="-999")
```
