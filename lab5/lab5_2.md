...
```r
fish %>%
  select(lakeid,radii_length_mm) %>% #choose column
  filter(lakeid=="AL" | lakeid=="AR") %>% #only values with lake id we want
  filter(radii_length_mm>2 & radii_length_mm<4) #filter even more that we want these lengths form those lakes
```
...
mammals %>% 
  select(genus, species, newborn) %>%
  mutate(newborn_new = ifelse(newborn == -999.00, NA, newborn))%>% #logical statement(newborn==-999. if it is true, newborn_new is na if it is false, then its a normal value so it is just set to the same value as newborn, or the original data.)
  arrange(newborn)
```
