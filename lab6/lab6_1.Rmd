...
```{r}
mammals %>% 
  mutate(across(c("order", "family"), tolower)) #only mutate these 2 columns
```
...
newmam<-
  ...
  mutate(max_life_new=ifelse(max_life== -999,NA,max_life)) #mutate(new variable name for column= manipulated original in some way OR ifelse(logical,if logical was true, if logical was false))
newmam
