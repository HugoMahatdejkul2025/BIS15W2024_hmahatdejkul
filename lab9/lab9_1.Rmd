...
We usually measure in wide data
## `pivot_longer()`  
...
```{r}
relig<-read_csv("data/relig_income.csv")
relig
```
...
```{r}
#Each column has its own data value and there is not a value for each input in each row.
```
