...
## [1] 17692    71
```
Very large data with lots of NAs
...
```r
  class(fisheries_tidy$asfis_species_number)
```
...
fisheries_tidy %>%
  ...
  group_by(country) %>%
  summarise(catch_total_sardines=sum(catch,na.rm=TRUE) ) %>%
  arrange(desc(catch_total_sardines)) 
```

```
## # A tibble: 37 × 2
##    country               catch_total_sardines
##    <fct>                                <dbl>
##  1 Morocco                               7470
##  2 Spain                                 3507
...
fisheries_tidy  %>%
  ...
  summarise(catch_tot=sum(catch,na.rm=TRUE)) %>%
  arrange(desc(catch_tot))
```

```
## # A tibble: 1,471 × 2
##    `(asfis_species_name)` catch_tot
##    <chr>                      <dbl>
...
```r
#which country had the most catches at any time after the year 2000?
fisheries_tidy %>%
  group_by(country) %>%
  select(common_name,country,year,catch)%>%
  filter(year>=2000)%>%
  summarise(catch_tot=sum(catch,na.rm=TRUE))%>%
  arrange(desc(catch_tot))%>%
  head(n=1)
