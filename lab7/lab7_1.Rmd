...
msleep %>%
  ...
  summarize(primatesmean=mean(bodywt),
            primatesmin=min(bodywt),
            primatesmax=max(bodywt),
            n())
...
msleep%>%
  ...
  summarize(n_genera=n_distinct(genus)) #this is going to count the number of genera in msleep. it will combine repeats
```
...
```{r}
msleep %>%
  summarize(minsleep=min(sleep_total),
            maxsleep=max(sleep_total),
            meansleep=mean(sleep_total),
            n())
```
...
msleep %>%
  group_by(order) %>%
  summarize(meanbrainweightbyorder=mean(brainwt)) 
