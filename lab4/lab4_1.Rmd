...
fishsubset<-select(fish,"fish_id","scalelength")
fishsubset
```
...
```{r}
select_if(fish, is.numeric) #super handy
```
...
```{r}
getwd()
mammal<-read_csv("mammal_lifehistories_v2.csv")
```
...
```{r}
names(mammal)[3]<-paste("genus")
names(mammal)[9]<-paste("mean mass")
names(mammal)[10]<-paste("afr")
names(mammal)[11]<-paste("max life")
mammal

#or use rename(mammals,genus="Genus", wean_mass="wean mass")
```
